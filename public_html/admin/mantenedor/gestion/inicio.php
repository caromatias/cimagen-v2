...
if(mysql_num_rows($res) > 0)
{
    ...
    while($reg = mysql_fetch_array($res))
    {
        ...
        $tEventos.="	<tr ".$style." onclick=\"window.location = 'inicio.php#".$reg[0]."';pasaUsuario();\" onContextMenu=\" menuTrUsuario(".$reg[0].");\";>
								...
								<td>
									<form action='delgestion.php' method='post' id='fdu".$reg[0]."'><input type='hidden' value='".$reg[0]."' name='Id'/></form>
									".$estado."
								</td>
							</tr>";
    }
    ...
    <script type="text/javascript">
        function pasaUsuario()
        {
            if(window.location.hash != '')
            {
                ...
                xmlhttp.open("GET",'agetgestion.php?idu=' + usr,true);
                xmlhttp.send();
            }
            else
            {
        ...
        {
            $("#sNombreUsuario").html('Nueva gestión');
            $("#hId").val('');
        ...
        {
            if(confirm('¿Realmente desea eliminar la gestión ' + id + '?') == true)
            {
        ...
        {
            $("#menu ul").html("<li onclick=\"nuevoUsuario();\">Nueva gestión</li>");
...
if(isset($_GET['m']))
{
    ...
    {
        case 'iok':
            echo "<script>mensaje('ok', 'Gestión creada correctamente.');</script>";
            break;
        case 'uok':
            echo "<script>mensaje('ok', 'Gestión actualizada correctamente.');</script>";
            break;
        case 'dok':
            echo "<script>mensaje('ok', 'Gestión eliminada correctamente.');</script>";
            break;
        case 'mua':
            echo "<script>mensaje('bad', 'No puede auto eliminarse.');</script>";
            break;
        case 'f':
            echo "<script>mensaje('bad', 'No se ha modificado ningnúna gestión.');</script>";
            break;
    }
}
?>
...
<div id="menu">
    <ul>
        <li onClick="nuevoUsuario();">Nueva gestión</li>
        <li onClick="location.reload();">Actualizar</li>
    </ul>
</div>
<center>
    <h1>Administración de "Gestiones de prensa"</h1>
    <div>
        <center>
            <form action="mangestion.php" method="post" name="fCreaUser" enctype="multipart/form-data">
