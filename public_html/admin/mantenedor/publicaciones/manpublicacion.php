...
if(isset($_POST['Id']) && $_POST['Id'] != 'null' && (int)$_POST['Id'] != 0)
{
    if(isset($_FILES['imagen']['size']) && $_FILES['imagen']['size'] != '' && isset($_FILES['pdf']['size']) && $_FILES['pdf']['size'] != '')
    {
        ...
        if ($archivo != "") {
            ...
            $destino =  "../../../recursos/publicaciones/pdfs/".$prefijo."_".$archivo;
            if (copy($_FILES['pdf']['tmp_name'],$destino)) {
                $status = "Archivo subido: <b>".$archivo."</b>";

                if ($archivoIm != "") {
                    // guardamos el archivo a la carpeta recursos/imagenes
                    $destinoIm =  "../../../recursos/publicaciones/".$prefijoIm."_".$archivoIm;
                    $destinoImTu = "../../../recursos/publicaciones/thumbs/".$prefijoIm."_".$archivoIm;
                    if (copy($_FILES['imagen']['tmp_name'],$destinoIm)) {
                        ...
                        $instruccion = "update publicaciones set nombre = '".$nombre."', rutaImg = '".$destinoIm."', rutaThumb = '".$destinoImTu."', estado = ".$estado.", rutaPdf = '".$destino."'  where id = ".$_POST['Id'].";";
    ...
    else
    {
        ...
        $nombre = $_POST['nombre'];
        $estado = $_POST['estado'];
        $query = "update publicaciones set nombre = '".$nombre."', estado = ".$estado." where id = ".$_POST['Id'].";";
        mysql_query($query);
...
else
{
    ...
    $prefijo = substr(md5(uniqid(rand())),0,6);
        ...
        if ($archivo != "") {
            ...
            crear_Thumbnail($destinoIm, 150, 150, 80, $destinoImTu);
            include "../../procesos/conectar.php";
            $instruccion = "insert into publicaciones (nombre, rutaImg, rutaThumb, estado, rutaPdf) values ('".$nombre."', '".$destinoIm."', '".$destinoImTu."', ".$estado.", '".$destino."');";
            $resul = mysql_query($instruccion) or die ("Problema en la consulta");
            if($resul == true)
            {
